...
def main(is_debug=False):
    ...
    # Project Saving Path
    project_path = args.project + '_{}_label_{}/'.format(args.backbone, args.labeled_percentage)#训练的时候要改名字，以便和ISIC数据集的结果分开
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            loss_x = (criterion_dsc(pred_u1A1_logits, pred_u2A2_pseudo.detach()) + criterion_dsc(pred_u2A2_logits, pred_u1A1_pseudo.detach())) / 2.#CPS
            loss_u = (criterion_dsc(pred_u1A1_logits, pred_u_pseudo.detach()) + criterion_dsc(pred_u2A2_logits, pred_u_pseudo.detach())) / 2.#CMT
        ...
        metrics.train_x_loss.append(np.mean(epoch_metrics.train_x_loss))#因为有Validation,所以二阶段的结果不需要添加log与writter？
        ############################
        # Validation
        ############################
