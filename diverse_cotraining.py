...
parser.add_argument('--config', type=str, required=True)                  #预训练配置文件？
...
parser.add_argument('--thr', default=0.95, type=float)                   #置信度阈值
...
def main():
    ...
    cfg = yaml.load(open(args.config, "r"), Loader=yaml.Loader)      #配置文件？读取yaml文件，网络运行的参数（如学习率等）
    cfg2 = yaml.load(open(args.config2, "r"), Loader=yaml.Loader)    #配置文件？
    cfg['conf_thresh'] = args.thr                                    #置信度阈值
    logger = init_log('global', logging.INFO)
    logger.propagate = 0

    rank, word_size = setup_distributed(port=args.port)              #多机多卡？word_size 一个job的全局进程数量，rank 进程的序号。例：两台八卡机器，group=1,word=2,word_size=8,第一台主机rank=0，显卡编号0～7，第二台rank=1，显卡编号0～7

    if rank == 0:                                                    #主机？
        all_args = {**cfg, **vars(args), 'ngpus': word_size}
        logger.info('{}\n'.format(pprint.pformat(all_args)))         #将所有参数写到info中

    if rank == 0:
        os.makedirs(args.save_path, exist_ok=True)                   #创建目录，exist_ok=True表示即使目录存在也不报错

    cudnn.enabled = True                                             #cuDNN的非确定性算法(NP)，等于True时启用，cuDNN设置为使用非确定性算法
    cudnn.benchmark = True                                           #在每次卷积前自动寻找最优的卷积算法并记录下来，适用于固定输入大小的情况，可以有变化，但不要太多。

    model1 = Segmentor(cfg)                                          #载入模型
    model2 = Segmentor(cfg2) # model2 uses dct
    if rank == 0:
        logger.info('Total params: {:.1f}M\n'.format(count_params(model1))) #计算模型参数总量，M是什么？
    ...
    local_rank = int(os.environ["LOCAL_RANK"])                                  #多卡训练？多机多卡？
    model1 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model1).cuda()       #将所有的 BatchNorm层转换为 SyncBatchNorm
    ...
    model1 = torch.nn.parallel.DistributedDataParallel(model1, device_ids=[local_rank],
                                                       output_device=local_rank, find_unused_parameters=False,broadcast_buffers=False)  #多机多卡或单机多卡的并行
    ...
    if cfg['criterion']['name'] == 'CELoss':
        ...
        criterion_l = ProbOhemCrossEntropy2d(**cfg['criterion']['kwargs']).cuda(local_rank) #在线难样本挖掘，针对不平衡数据分布和困难样本训练的一种改进版本的交叉熵损失函数
    else:
        raise NotImplementedError('%s criterion is not implemented' % cfg['criterion']['name'])

    criterion_u = nn.CrossEntropyLoss(reduction='none').cuda(local_rank)  #reduction可设置为mean, sum, none，默认为 mean表示损失为多个样本的平均值，sum表示损失的和，none表示不综合
