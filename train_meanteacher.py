...
def main(is_debug=False):
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        for idx in pbar:
            ...
            # CPS loss
            loss1_u = criterion_u(pred1_u, pseudo.detach())
            # loss2_u = criterion_u(pred2_u, pseudo1.detach())
            loss_cps = loss1_u
            loss_u = loss_cps * alpha
            ...
            entropy1 = entropy(pred1_u_feature)
            entropies1.append(entropy1.mean().item())
            # entropy2 = entropy(pred2_u_feature)
            # entropies2.append(entropy2.mean().item())
            entropy3 = entropy(pred_u_feature)
            ...
            if idx % args.log_freq == 0:
                logger.info("Train1: Epoch/Epochs {}/{}\t"
                            "iter/iters {}/{}\t"
                            "loss {:.3f}, loss_sup {:.3f}, loss_cps {:.3f}, lambda {}".format(epoch,
                                                                                                               args.num_epochs,
                                                                                                               idx,
                                                                                                               len(pbar),
                                                                                                               loss.item(),
            ...
            pred1_u = student1(uimage)['out']
            # pred2_u = student2(uimage2)['out']

            pseudo1_softmax = torch.softmax(pred1_u, dim=1)
            pseudo1 = torch.argmax(pseudo1_softmax, dim=1)
            # pseudo2_softmax = torch.softmax(pred2_u, dim=1)
            # pseudo2 = torch.argmax(pseudo2_softmax, dim=1)
            # # CMT loss
            # loss1_cmt = criterion_c(pred1_u, pseudo.detach())
            # loss2_cmt = criterion_c(pred2_u, pseudo.detach())
            # loss_cmt = (loss1_cmt + loss2_cmt) * 0.5
            ...
            entropy1 = entropy(pseudo1_softmax)
            entropies1.append(entropy1.mean().item())
            # entropy2 = entropy(pseudo2_softmax)
            # entropies2.append(entropy2.mean().item())
            entropy3 = entropy(pred_u_feature)
            entropies3.append(entropy3.mean().item())
