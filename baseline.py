...
#    rank, word_size = setup_distributed(port=args.port)
    ...
    model1 = Segmentor(cfg3).to(device)
    model2 = Segmentor(cfg3).to(device)
...
###############################################################################################################################################
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        iter_train_labeled_dataloader = iter(train_labeled15_dataloader)
        iter_train_unlabeled_dataloader = iter(train_unlabeled_dataloader)
...
#更改部分####################################################################################################################
            ...
            pred1_feature = torch.softmax(pred1, dim=1)
            loss1_sup = criterion(pred1, label.squeeze(1).long())  # [4,2,256,256]  [4,256,256]
            loss_sup1 = loss11_sup * 9 + loss22_sup + loss1_sup
...
#             loss_cps1 = loss1_u_total + loss11_u_total + loss22_u_total
            ...
            if idx % args.log_freq == 0:
                logger.info("Train1: Epoch/Epochs {}/{}\t"
                            "iter/iters {}/{}\t"
                            "loss1 {:.3f}, loss_sup1 {:.3f}".format(epoch, args.num_epochs, idx, len(pbar),
            ...
            pred11_stage2 = model1(image)
            # pred22_stage2 = model2(image2_dct, ([256, 256]))
            # pred22_stage2 = model2(image2)
            pred22_stage2 = model2(image)
            # pred1_stage2 = model3(image2)
            pred1_stage2 = model3(image)
            ...
            loss11_sup = criterion(pred11_stage2, label.squeeze(1).long())  ###################################################修改后的loss,损失函数好要用criterion_l？？？？？？？？？？？？？？？
            loss22_sup = criterion(pred22_stage2, label.squeeze(1).long())  #################################################修改后的loss,损失函数好要用criterion_l？？？？？？？？？？？？？？？？
            loss1_sup = criterion(pred1_stage2, label.squeeze(1).long())
            loss_sup =  loss11_sup * 9 + loss22_sup + loss1_sup
            #无监督
            # with torch.no_grad():
            ...
            # pred11_u_stage2 = model1(uimage)
            # # pred22_u_stage2 = model2(uimage2_dct, ([256, 256]))  #################################################
            # pred22_u_stage2 = model2(uimage)
            # #pred1_u_stage2 = model3(uimage2)
            # pred1_u_stage2 = model3(uimage)
...
#更改部分到此结束##################################################################################################################################################
            ...
            #     image2_dct = torch.cat(image2_dct_expanded_to_concat, dim=0)                              ############得到dct转换后的tensor image2_dct
            #     image2_dct = image2_dct.cuda()
            #     # print(tensors[0].squeeze(0))#[4 64 64 64]
            #     # return 0
            #
            #
            #
            # #有监督的路径下得到了交换后的图像image2，label2######################################################################
            # # if self.mode == 'train_l':
            # #     img_dct = self.dct_transform(deepcopy(img))  # DCTtransform
            # #     mask = torch.from_numpy(np.array(mask)).long()  # 将数组转化为张量
            # #     return img_dct, normalize(img), mask, img_shape  # normalize中针对image的mean和std作了处理
            # #image2_dct =image2.dct##############################################################################3
            # pred1s = student1(image2)
            # pred2s = student2(image2)
            # pred1 = pred1s['out']   #[4 2 256 256]
            # pred11=model1(image2)##############################################[4 2 256 256]                                   模型的更新放在哪里?
            # # print(type(label2.squeeze(1).long()))
            # # return 0
            ...
            # loss11_sup = criterion(pred11,label2.squeeze(1).long())###################################################修改后的loss,损失函数好要用criterion_l？？？？？？？？？？？？？？？
            # loss2_sup = criterion(pred2, label2.squeeze(1).long())
            # loss22_sup = criterion(pred22, label2.squeeze(1).long())#################################################修改后的loss,损失函数好要用criterion_l？？？？？？？？？？？？？？？？
            # loss_sup = loss1_sup + loss2_sup + loss11_sup + loss22_sup
            #
            # ###########################
            #     # unsupervised path #
            # ###########################
            ...
            # pred11_u = model1(uimage2)##################################################
            # pred2_u = student2(uimage2)['out']
            # pred22_u = model2(uimage2_dct,([256,256]))#################################################
            #
            ...
            # mask_u_w_cutmixed1, conf_u_w_cutmixed1 = mask_u_w1.clone(), conf_u_w1.clone()
            # mask_u_w_cutmixed2, conf_u_w_cutmixed2 = mask_u_w2.clone(), conf_u_w2.clone()
            # # ignore_mask_cutmixed1, ignore_mask_cutmixed2 = \
            # #     ignore_mask.clone(), ignore_mask.clone()                                        #???????????????????ignore_mask?????????????????????
            # #
            ...
            # loss2_cmt = criterion_c(pred2_u, pseudo.detach())
            # loss11_cmt= criterion_c(pred11_u, pseudo.detach())
            # loss22_cmt = criterion_c(pred22_u, pseudo.detach())
            # loss_cmt = (loss1_cmt + loss2_cmt + loss11_cmt + loss22_cmt) * 0.25                                     #??CMT需要加上新增的两个吗？？？？？？？？？？
            #
            # # CPS loss
            # loss1_u = criterion_u(pred1_u, pseudo2.detach())
            # loss111_u=criterion_u(pred1_u, mask_u_w_cutmixed1.detach())
            # loss122_u=criterion_u(pred1_u, mask_u_w_cutmixed2.detach())
            # loss1_u=(loss1_u*3+loss111_u+loss122_u)/3.0
            #
            # loss2_u = criterion_u(pred2_u, pseudo1.detach())
            # loss211_u=criterion_u(pred2_u, mask_u_w_cutmixed1.detach())
            # loss222_u = criterion_u(pred2_u, mask_u_w_cutmixed2.detach())
            # loss2_u = (loss2_u*3 + loss211_u + loss222_u) / 3.0
            #
            # loss_u_s12 = criterion_u(pred11_u, mask_u_w_cutmixed2)
            # # loss_u_s12 = loss_u_s12 * ((conf_u_w_cutmixed2 >= cfg['conf_thresh']) & (ignore_mask_cutmixed1 != 255))
            # # loss_u_s12 = torch.sum(loss_u_s12) / torch.sum(ignore_mask_cutmixed1 != 255).item()
            # loss_u_s111=criterion_u(pred11_u, pseudo1.detach())
            # loss_u_s112=criterion_u(pred11_u, pseudo2.detach())
            # loss11_u=(loss_u_s12*3+loss_u_s111+loss_u_s112)/3.0
            #
            # loss_u_s21 = criterion_u(pred22_u, mask_u_w_cutmixed1)
            # # loss_u_s21 = loss_u_s21 * ((conf_u_w_cutmixed1 >= cfg['conf_thresh']) & (ignore_mask_cutmixed2 != 255))
            # # loss_u_s21 = torch.sum(loss_u_s21) / torch.sum(ignore_mask_cutmixed2 != 255).item()
            # loss_u_s221=criterion_u(pred22_u, pseudo1.detach())
            # loss_u_s222 = criterion_u(pred22_u, pseudo2.detach())
            # loss22_u = (loss_u_s21*3 + loss_u_s221 + loss_u_s222) / 3.0
            #
            #
            #
            # loss_cps = (loss1_u + loss2_u+loss11_u+loss22_u) * 0.25
            # #torch.distributed.barrier()
...
#一直注释到这里###########################################################################################################################################################
            ...
            #         epoch, args.num_epochs, idx, len(pbar),
            #         loss.item(), loss_sup.item(), loss_cps2.item(), lambda_))
            # if idx % args.log_freq == 0:
            #     logger.info("Train2: Epoch/Epochs {}/{}\t"
            #                 "iter/iters {}/{}\t"
            ...
            if idx % args.log_freq == 0:
                logger.info("Train2: Epoch/Epochs {}/{}\t"
                            "iter/iters {}/{}\t"
                            "loss {:.3f}, loss_sup {:.3f}".format(
                    epoch, args.num_epochs, idx, len(pbar),
                    loss.item(), loss_sup.item()))
...
###第一行应该缩进到第二个竖线那里#epoch的轮次要注意#在新添加的这段里面找到类似pred=model(image)的代码，然后根上下两部分拼上，同时还应该注意这里的模型不算UCMT中第一阶段训练过的模型，应该根据diverse cotraining的特殊性进行其他修改##########################################################################################
    # for epoch in range(cfg['epochs']):
    #     if rank == 0:
    #         logger.info('===========> Epoch: {:}, LR: {:.4f}, '
    #                     'Previous best1: {:.2f}, Previous best2: {:.2f}, Previous best3: {:.2f}'.format(
    #             epoch, optimizer11.param_groups[0]['lr'], previous_best1, previous_best2, previous_best3))
    #
    ...
    #             (img_u_w, img_u_s, img_u_w_dct, img_u_s_dct, ignore_mask, _, cutmix_box_dct_img, cutmix_box, _,
    ...
    #             pred_x2, pred_u_w2 = model2(torch.cat((img_x_dct, img_u_w_dct)), img_shape[0]).split(
    #                 [num_lb, num_ulb])
    ...
    #             loss_u_s11 = loss_u_s11 * (
    #                     (conf_u_w_cutmixed2 >= cfg['conf_thresh']) & (ignore_mask_cutmixed1 != 255))
    ...
    #             loss_u_s12 = loss_u_s12 * (
    #                     (conf_u_w_cutmixed3 >= cfg['conf_thresh']) & (ignore_mask_cutmixed1 != 255))
    #             loss_u_s12 = torch.sum(loss_u_s12) / torch.sum(ignore_mask_cutmixed1 != 255).item()
    ...
    #             loss_u_s21 = loss_u_s21 * (
    #                     (conf_u_w_cutmixed1 >= cfg['conf_thresh']) & (ignore_mask_cutmixed2 != 255))
    #             loss_u_s21 = torch.sum(loss_u_s21) / torch.sum(ignore_mask_cutmixed2 != 255).item()
    #
    #             loss_u_s22 = criterion_u(pred_u_s2, mask_u_w_cutmixed3)
    #             loss_u_s22 = loss_u_s22 * (
    #                     (conf_u_w_cutmixed3 >= cfg['conf_thresh']) & (ignore_mask_cutmixed2 != 255))
    ...
    #             loss_u_s31 = loss_u_s31 * (
    #                     (conf_u_w_cutmixed1 >= cfg['conf_thresh']) & (ignore_mask_cutmixed3 != 255))
    ...
    #             loss_u_s32 = loss_u_s32 * (
    #                     (mask_u_w_cutmixed2 >= cfg['conf_thresh']) & (ignore_mask_cutmixed3 != 255))
    ...
    #             loss.backward()
    #
    #             scaler1.step(optimizer11)
    #             scaler2.step(optimizer22)
    ...
    #             (((mask_u_w2 == mask_u_w1) & (mask_u_w2 == mask_u_w3)) * (
    #                     (conf_u_w1 >= cfg['conf_thresh']) & (conf_u_w2 >= cfg['conf_thresh']) & (
    #                     conf_u_w3 >= cfg['conf_thresh']) & (ignore_mask != 255))).sum().item() / \
    #             ((conf_u_w1 >= cfg['conf_thresh']) & (conf_u_w2 >= cfg['conf_thresh']) & (
    #                     ignore_mask != 255)).sum().item()
    #
    #         agree_ratio12 = ((mask_u_w2 == mask_u_w1) * (
    #                 (conf_u_w1 >= cfg['conf_thresh']) & (conf_u_w2 >= cfg['conf_thresh']) & (
    #                 ignore_mask != 255))).sum().item() / \
    #                         ((conf_u_w1 >= cfg['conf_thresh']) & (conf_u_w2 >= cfg['conf_thresh']) & (
    #                                 ignore_mask != 255)).sum().item()
    #         agree_ratio23 = ((mask_u_w2 == mask_u_w3) * (
    #                 (conf_u_w3 >= cfg['conf_thresh']) & (conf_u_w2 >= cfg['conf_thresh']) & (
    #                 ignore_mask != 255))).sum().item() / \
    #                         ((conf_u_w3 >= cfg['conf_thresh']) & (conf_u_w2 >= cfg['conf_thresh']) & (
    #                                 ignore_mask != 255)).sum().item()
    #         agree_ratio13 = ((mask_u_w1 == mask_u_w3) * (
    #                 (conf_u_w3 >= cfg['conf_thresh']) & (conf_u_w1 >= cfg['conf_thresh']) & (
    #                 ignore_mask != 255))).sum().item() / \
    #                         ((conf_u_w3 >= cfg['conf_thresh']) & (conf_u_w1 >= cfg['conf_thresh']) & (
    #                                 ignore_mask != 255)).sum().item()
    #
    ...
    #                     total_loss_x2 / (i + 1), total_loss_s2 / (i + 1), total_mask_ratio2 / (i + 1),
    #                     total_agree_ratio_between3 / (i + 1), total_agree_ratio_between2 / (i + 1)))
    #
    ...
    #                         'IoU2: {:.2f}, IoU3: {:.2f},'.format(cls_idx, CLASSES[cfg['dataset']][cls_idx], iou,
    #                                                                      iou_class2[cls_idx], iou_class3[cls_idx]))
    #         logger.info(
    #             '***** Evaluation {} ***** >>>> MeanIoU1: {:.2f}, MeanIoU2: {:.2f}, MeanIoU3: {:.2f}\n'.format(
    #                 eval_mode, mIOU1, mIOU2, mIOU3))
    #
    #     if mIOU1 > previous_best1 and rank == 0:
    #         if previous_best1 != 0:
    #             pre_path = os.path.join(args.save_path, 'm1_%s_%.2f.pth' % (cfg['backbone'], previous_best1))
