...
###############################################################################################################################################
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        def show_image(image, label):
            # 注意：这里假设image是一个PIL图像或者是一个在[0, 1]范围内的Tensor
            # 如果你的图像在[0, 255]的整数范围内，你需要先将其转换到[0, 1]的浮点数范围
            if isinstance(image, torch.Tensor):
                image = image.cpu().permute(1, 2, 0).numpy()  # HWC, 假设图像是CHW格式的Tensor
                image = transforms.ToPILImage()(image)  # 将Tensor转换为PIL图像
...
#更改部分####################################################################################################################
            ...
            for i in range(j):
                # print(image[i])
                # show_image(image[i], label[i])
                single_image = image[i]  # type torch.tensor   shape [3 256 256]

                # single_image_show=sigmoid(single_image.cpu())
                # pil_image_show = ToPILImage()(single_image_show)

                pil_image = ToPILImage()(single_image)
                # upsampled_2x = pil_image_show.resize((pil_image.width * 2, pil_image.height * 2), Image.ANTIALIAS)
                #
                # # 对PIL图像进行四倍上采样
                # upsampled_4x = pil_image_show.resize((pil_image.width * 4, pil_image.height * 4), Image.ANTIALIAS)
                #
                # # 可视化原始图像
                # pil_image_show.show()
                #
            ...
            for i in range(m):
                ...
                save_folder_h = '/home/li/桌面/UCMT-main/images/h1/'  # 确保这个文件夹已经存在
                image_path_h = save_folder_h + str(epoch) + str(idx) + str(i) + 'black_white_image.jpg'
                # 使用matplotlib保存图像为jpg格式
                plt.imsave(image_path_h, image_to_save_h, cmap='gray')

                black_white_image_h = (pseudo22_l == 1).float()  # 这将返回一个形状相同的浮点张量，其中1表示白色，0表示黑色
                # 如果需要，可以将浮点张量缩放到0-255的整数范围
                black_white_image_h_uint8 = (black_white_image_h * 255).byte()
                # 选择一个图像进行可视化（如果有多个图像在批量中）
                image_to_save_h = black_white_image_h_uint8[i]  # 取第一个图像作为示例
                image_to_save_h = image_to_save_h.cpu()
                save_folder_h = '/home/li/桌面/UCMT-main/images/f21/'  # 确保这个文件夹已经存在
                image_path_h = save_folder_h + str(epoch) + str(idx) + str(i) + 'black_white_image.jpg'
                # 使用matplotlib保存图像为jpg格式
                plt.imsave(image_path_h, image_to_save_h, cmap='gray')

                black_white_image_h = (pseudo1_l == 1).float()  # 这将返回一个形状相同的浮点张量，其中1表示白色，0表示黑色
                # 如果需要，可以将浮点张量缩放到0-255的整数范围
                black_white_image_h_uint8 = (black_white_image_h * 255).byte()
                # 选择一个图像进行可视化（如果有多个图像在批量中）
                image_to_save_h = black_white_image_h_uint8[i]  # 取第一个图像作为示例
                image_to_save_h = image_to_save_h.cpu()
                save_folder_h = '/home/li/桌面/UCMT-main/images/f31/'  # 确保这个文件夹已经存在
                image_path_h = save_folder_h + str(epoch) + str(idx) + str(i) + 'black_white_image.jpg'
                # 使用matplotlib保存图像为jpg格式
                plt.imsave(image_path_h, image_to_save_h, cmap='gray')
...
#无cmt的训练################################################################################################################
            ...
            # tensors = torch.chunk(image2, 4, dim=0)
                ...
                for i in range(l):
                    ...
                    image_path_uncertainty_map11 = save_folder_uncertainty_map11 +str(epoch)+ str(idx) + str(i) + 'black_white_image.jpg'
                    save_folder_uncertainty_map22 = '/home/li/桌面/UCMT-main/images/uncertainty_map22/'
                    image_path_uncertainty_map22 = save_folder_uncertainty_map22 +str(epoch)+ str(idx) + str(i) + 'black_white_image.jpg'
                    # 如果你想要保存图像
                    cv2.imwrite(image_path_uncertainty_map11, colormap11)
                    cv2.imwrite(image_path_uncertainty_map22, colormap22)
                # img_img = sigmoid(img_img)
                # print(img_img[0])
                # return 0
                # mlm = 0
                for i in range(l):
                    ...
                    # img=(img+255)/255.0
                    image_path =str(epoch) + str(idx) + str(i) + 'image.png'
                    ...
                    # 使用 Matplotlib 保存图像
                    plt.imsave(filename, img)  # 对于彩色图像，不需要指定 cmap
                # mlm=mlm+1
                for i in range(l):
            ...
            for i in range(l):
                black_white_image_h = (pseudo11_l2 == 1).float()  # 这将返回一个形状相同的浮点张量，其中1表示白色，0表示黑色
                # 如果需要，可以将浮点张量缩放到0-255的整数范围
                black_white_image_h_uint8 = (black_white_image_h * 255).byte()
                # 选择一个图像进行可视化（如果有多个图像在批量中）
                image_to_save_h = black_white_image_h_uint8[i]  # 取第一个图像作为示例
                ...
                black_white_image_h = (pseudo22_l2 == 1).float()  # 这将返回一个形状相同的浮点张量，其中1表示白色，0表示黑色
                # 如果需要，可以将浮点张量缩放到0-255的整数范围
                black_white_image_h_uint8 = (black_white_image_h * 255).byte()
                # 选择一个图像进行可视化（如果有多个图像在批量中）
                image_to_save_h = black_white_image_h_uint8[i]  # 取第一个图像作为示例
                ...
                black_white_image_h = (pseudo1_l2 == 1).float()  # 这将返回一个形状相同的浮点张量，其中1表示白色，0表示黑色
                # 如果需要，可以将浮点张量缩放到0-255的整数范围
                black_white_image_h_uint8 = (black_white_image_h * 255).byte()
                # 选择一个图像进行可视化（如果有多个图像在批量中）
                image_to_save_h = black_white_image_h_uint8[i]  # 取第一个图像作为示例
                image_to_save_h = image_to_save_h.cpu()
                save_folder_h = '/home/li/桌面/UCMT-main/images/f3/'  # 确保这个文件夹已经存在
                image_path_h = save_folder_h + str(epoch) + str(idx) + str(i) + 'black_white_image.jpg'
                # 使用matplotlib保存图像为jpg格式
                plt.imsave(image_path_h, image_to_save_h, cmap='gray')
            loss11_cps_l2 = (criterion_u(pred11_stage2, pseudo22_l2.detach()) + criterion_u(pred11_stage2, pseudo1_l2.detach())) / 2.
            loss22_cps_l2 = (criterion_u(pred22_stage2, pseudo11_l2.detach()) + criterion_u(pred22_stage2, pseudo1_l2.detach())) / 2.
            loss1_cps_l2 = (criterion_u(pred1_stage2, pseudo11_l2.detach()) + criterion_u(pred1_stage2, pseudo22_l2.detach())) / 2.
            ...
            with torch.no_grad():
                ...
                imgu_unfold = unfolds(uimage).view(B, C, h, w, -1)  # B x C x h x w x L  [4,3,16,16,256]
                ...
                j=uimage2.size(0)
                save_dir = '/home/li/桌面/UCMT-main/images/uimage2/'
                img_img = uimage2.cpu()
                # img_img = sigmoid(img_img)
                # print(img_img[0])
                # return 0
                ...
                for i in range(j):
                    ...
                    # img = img.astype(np.float32)
                    # img=(img+255)/255.0
                    image_path =str(epoch) + str(idx) + str(j) + 'image.png'
